...
class Game:
    ...
    def stats_screen(self):
        ...
        pygame.draw.rect(self.screen, WHITE,
                         (WIDTH // 8, BORDER, WIDTH * 3 // 4, HEIGHT - BORDER * 6 - self.back_button.rect.height))
        ...
        for i in range(len(self.stats)):
            ...
            if self.stat_total > 0:
                pygame.draw.rect(self.screen, BLUE, (WIDTH // 8 + BORDER * 7, y,
                                                     (WIDTH * 3 // 4 - BORDER * 13) * self.stats[i] // self.stat_total,
                                                     48))
                draw_text(self.screen, str(self.stats[i]), 48, BLACK,
                          WIDTH // 8 + BORDER * 6 + (WIDTH * 3 // 4 - BORDER * 13) * self.stats[
                              i] // self.stat_total + BORDER * 2, y, 'topleft')
            else:
                draw_text(self.screen, str(self.stats[i]), 48, BLACK, WIDTH // 8 + BORDER * 7 + BORDER * 2, y,
                          'topleft')
            y += BORDER + 48
    ...
    def end_screen(self):
        ...
        draw_text(self.screen, "THE WORD WAS " + self.board.word_str, 32, WHITE, WIDTH // 2, SIDE + BORDER * 2 + 48,
                  'midtop')
        if self.board.round < self.board.num_rounds:
            if self.board.round + 1 == 1:
                draw_text(self.screen, "YOU WON IN " + str(self.board.round + 1) + " ROUND", 32, WHITE, WIDTH // 2,
                          SIDE + BORDER * 3 + 48 + 32, 'midtop')
            else:
                draw_text(self.screen, "YOU WON IN " + str(self.board.round + 1) + " ROUNDS", 32, WHITE, WIDTH // 2,
                          SIDE + BORDER * 3 + 48 + 32, 'midtop')

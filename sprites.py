    ...
    def update(self):
        for i in range(len(self.tiles)):
            for j in range(len(self.tiles[i])):
                start_x = WIDTH // 2 - (
                        int(self.num_chars / 2 * self.TILE_SIZE) + (self.num_chars // 2 + 1) * self.BORDER)

                pygame.draw.rect(self.image, self.color_tiles[i][j],
                ...
                if self.tiles[i][j] is not None:
                    if i == self.round:
                        draw_text(self.image, self.tiles[i][j], 32, self.current_round_color,
                        ...
                        draw_text(self.image, self.tiles[i][j], 32, BLACK,
                                  start_x + self.BORDER + j * (self.TILE_SIZE + self.BORDER) + self.TILE_SIZE / 2,
                                  self.BORDER + i * (self.TILE_SIZE + self.BORDER) + self.TILE_SIZE / 2, 'center')
